...
- **`src/container.rs`** — Podman container management for SPT server lifecycle via bollard. Default SPT server image: `ghcr.io/cebarks/quartermaster/spt-server:latest`. Default headless client image: `localhost/fika-headless:latest` (configurable via `headless.image` in config).
