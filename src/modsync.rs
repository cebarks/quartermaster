...
pub fn regenerate_if_enabled(spt_dir: &Path, config: &Config, db: &Database) -> Result<bool> {
    ...
    let output = generate_config(ms_config, db, Some(spt_dir))?;
...
pub fn preview_config(
    ...
    let output = generate_config(ms_config, db, spt_dir)?;
...
mod tests {
    ...
    fn generate_config_client_mod_creates_sync_path() {
        ...
        let output = generate_config(&ms_config, &db, None).unwrap();
    ...
    #[test]
    fn generate_config_extra_sync_paths_enabled() {
        let db = Database::open_in_memory().unwrap();
    ...
    fn generate_config_patcher_dir_synced_when_all_files_tracked() {
        ...
        let ms_config = ModSyncConfig::default();
        let output = generate_config(&ms_config, &db, Some(spt_dir)).unwrap();
